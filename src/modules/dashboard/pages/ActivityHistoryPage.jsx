...
export function ActivityHistoryPage({ onNavigate }) {
  ...
  return <section className="module-page page-enter">
    ...
    <Modal open={Boolean(reviewActivity)} onClose={() => !saving && setReviewActivity(null)} title="Beri Rating Fasilitator">
      {reviewActivity && <form onSubmit={submitReview}><div className="rating-hero"><div className="rating-hero-icon">★</div><div><strong>Bagaimana pengalaman Anda?</strong><span>Berikan penilaian untuk membantu meningkatkan kualitas fasilitator.</span></div></div><p className="muted" style={{ fontSize: 13 }}>Rating untuk <strong>{formatFacilitatorName(reviewActivity.facilitator)}</strong> pada kegiatan <strong>{reviewActivity.name}</strong>.</p>{reviewError && <div className="form-error" style={{ marginBottom: 10 }}>{reviewError}</div>}<label className="form-field"><span>Rating</span><select value={rating} onChange={(event) => setRating(event.target.value)}>{[5, 4, 3, 2, 1].map((value) => <option key={value} value={value}>{value} bintang</option>)}</select></label><div className="modal-footer rating-form-actions"><button className="primary-button" disabled={saving}>{saving ? 'Menyimpan...' : 'Simpan Rating'}</button><button type="button" className="outline-button" onClick={() => setReviewActivity(null)} disabled={saving}>Batal</button></div></form>}
    </Modal>
  </section>
}
